...
from (
    select CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, STREET, CITY,STATE,COUNTRY,
      ...
      from (select CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, STREET, CITY,STATE,COUNTRY,UPDATE_TIMESTAMP
            from manage_db.external_stages.customer_table_changes
...
delete from customer where customer_id = 136 or FIRST_NAME = 'Kim';
select count(*) cnt, customer_id from customer group by customer_id having cnt = 1;
...
select * from customer_history where is_current = false order by end_time desc;
show tasks;